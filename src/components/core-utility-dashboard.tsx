...
export function CoreUtilityDashboard({ projectId, module }: { projectId: string; module: "leads-finder" | "accman" }) {
  const [records, setRecords] = useState<UtilityRecord[]>([]);
  const [status, setStatus] = useState("Loading project records...");
  const research = module === "leads-finder";
  const [recordType, setRecordType] = useState(research ? "signal" : "account");
  ...
  async function load() {
    ...
    setRecords((payload.records ?? []).filter((record: UtilityRecord) =>
      record.module === module || (research && record.module === "aclienti"),
    ));
    setStatus("");
  }
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const payload = research ? { pain: values.painText, sourceTitle: values.sourceTitle, sourceUrl: values.sourceUrl, scores: { pain: Number(values.pain), fit: Number(values.fit), timing: Number(values.timing), reach: Number(values.reach), evidence: Number(values.evidence) }, qualificationScore: score } : { platform: values.platform, notes: values.notes, status: values.status };
    setStatus("Saving to the authenticated project...");
    const response = await fetch(`/api/projects/${projectId}/modules`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "upsert_record", module, recordType, externalId: `${recordType}-${Date.now()}`, name, status: research ? (score >= 65 ? "qualified" : "new") : String(values.status || "idea"), sourceRepo: research ? "DistroNow Leads Finder research" : "DistroNow accman utility", payload }) });
  ...
  return (
    <section className="core-utility-dashboard">
      <div className="core-utility-header"><div><p className="eyebrow">Project-owned utility</p><h2>{research ? "Find better customer-backed opportunities." : "Run the account and content planning loop."}</h2><p>{research ? "Capture public evidence, score it, and turn qualified signals into buyer or content briefs inside Leads Finder." : "Keep accounts, niches, formats, trends, prompts, and plans in one authenticated workspace."}</p></div><span>{status}</span></div>
