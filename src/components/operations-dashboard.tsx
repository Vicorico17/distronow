...
export function OperationsDashboard({ projectId }: OperationsDashboardProps) {
  ...
  useEffect(() => {
    // This effect synchronizes the screen with the project operations API.
      ...
      action === "save_strategy"
        ...
        : action === "add_signal"
          ? {
              ...
              fitScore: Number(value.fitScore),
              painScore: Number(value.painScore),
              timingScore: Number(value.timingScore),
              reachabilityScore: Number(value.reachabilityScore),
              evidenceScore: Number(value.evidenceScore),
            }
          : action === "add_prospect"
            ? {
                action,
              ...
              : action === "schedule_post"
                ...
                : action === "add_metric"
                  ? {
                      ...
                      engagements: Number(value.engagements),
  ...
  return (
    <section className="operations-dashboard">
      <div className="operations-header">
        <div>
          <p className="eyebrow">Shared operating records</p>
          <h2>Make the next decision with evidence.</h2>
          <p>
            These records connect customer research, strategy, acquisition, and
            account operations.
          </p>
        </div>
      ...
      <div className="operations-stats">
        ...
        <div>
          <strong>{data.prospects.length}</strong>
          <span>Prospects</span>
        </div>
        <div>
      ...
      <div className="operations-grid">
        <form
          ...
          onSubmit={(event) => submit(event, "save_strategy")}
        >
          <div className="operation-panel-head">
            <span>DistroNow</span>
            <small>Strategy Library</small>
          </div>
          <h3>Confirm the strategy before creating.</h3>
          <select
            value={strategyType}
          ...
          <textarea
            name="content"
            placeholder="Write or paste the decision this document should preserve..."
            required
          />
          <select name="status" defaultValue="review">
        ...
        >
          ...
          <textarea
            name="evidence"
            placeholder="What did the customer actually say or do?"
            required
          />
          <div className="operation-form-row">
            <input name="sourceName" placeholder="Source / community" />
            <input
              name="sourceUrl"
          ...
          <button className="primary-action" type="submit">
            Save customer signal
          </button>
        </form>
        <form
          className="operation-panel"
          onSubmit={(event) => submit(event, "add_prospect")}
        >
          <div className="operation-panel-head">
            <span>Leads Finder</span>
            <small>Buyer Intelligence &amp; Outreach</small>
          </div>
          <h3>Review a prospect before outreach.</h3>
          <input name="companyName" placeholder="Company name" required />
          <div className="operation-form-row">
            <input name="contactName" placeholder="Contact name" />
            <input name="contactRole" placeholder="Role" />
          </div>
          <div className="operation-form-row">
          ...
          <div className="operation-form-row">
            ...
            <input name="sourceUrl" placeholder="Evidence URL" type="url" />
          </div>
          <button className="primary-action" type="submit">
            Add prospect for review
          </button>
        </form>
        <form
          className="operation-panel"
          onSubmit={(event) => submit(event, "create_task")}
        >
          <div className="operation-panel-head">
        ...
        <form
          className="operation-panel"
          onSubmit={(event) => submit(event, "add_account")}
        >
          <div className="operation-panel-head">
            <span>accman</span>
            <small>Account Manager</small>
          </div>
          <h3>Add a channel to the plan.</h3>
          <div className="operation-form-row">
        ...
        <form
          className="operation-panel"
          onSubmit={(event) => submit(event, "schedule_post")}
        >
          <div className="operation-panel-head">
            <span>accman</span>
            <small>Publishing queue</small>
          </div>
          <h3>Move approved work into distribution.</h3>
          <input name="title" placeholder="Post or campaign title" required />
          <div className="operation-form-row">
            <select name="platform" defaultValue="Instagram">
              <option>Instagram</option>
              <option>TikTok</option>
          ...
          <textarea
            name="notes"
            placeholder="Account, CTA, or publishing notes"
          />
          <button className="primary-action" type="submit">
            Add to queue
          </button>
        ...
        <form
          className="operation-panel"
          onSubmit={(event) => submit(event, "add_metric")}
        >
          <div className="operation-panel-head">
            <span>Analytics</span>
            <small>Performance loop</small>
          </div>
          ...
          <div className="operation-form-row">
            <input name="clicks" type="number" min="0" placeholder="Clicks" />
            <input
              name="conversions"
              type="number"
              min="0"
      ...
      <div className="operations-lists">
        <section>
          <div className="operation-list-head">
            ...
            <span>{data.signals.length}</span>
          </div>
          {data.signals.slice(0, 5).map((signal) => (
            <article className="operation-list-item" key={signal.id}>
              <strong>{signal.title}</strong>
              <p>{signal.evidence}</p>
              <small>
                {signal.source_name ?? "Manual"} · Fit {signal.fit_score}/5 ·{" "}
                {signal.status}
              </small>
            </article>
          ))}
        ...
        <section>
          <div className="operation-list-head">
            <h3>Prospects</h3>
            <span>{data.prospects.length}</span>
          </div>
          {data.prospects.slice(0, 5).map((prospect) => (
            <article className="operation-list-item" key={prospect.id}>
              <strong>{prospect.company_name}</strong>
              <p>
                {prospect.contact_name ?? "Contact not yet assigned"}{" "}
                {prospect.contact_role ? `· ${prospect.contact_role}` : ""}
              </p>
              <small>
                Fit {prospect.fit_score}/100 · {prospect.status}
              </small>
            </article>
          ))}
        </section>
